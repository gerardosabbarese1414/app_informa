...
def day_page(user_id: int, d: date):
    ...
    c2.metric(
        "Calorie OUT (workout)",
        f"{kcal_round(workouts_df['calories_burned'].sum())} kcal" if not workouts_df.empty else "0 kcal",
    )
    ...
    st.divider()
    st.subheader("➕ Aggiungi (AI o manuale)")

    tab1, tab2, tab3, tab4 = st.tabs(
        ["🧠 Pasto da testo (AI)", "📷 Pasto da foto (AI)", "✍️ Pasto manuale", "🏃 Allenamento (AI o manuale)"]
    )
    ...
    with tab1:
        ...
        if est:
            ...
            adj_kcal = st.number_input(
                "Kcal (modificabile)", min_value=0, value=int(round(float(est.get("total_calories") or 0))), step=10,
                key=f"meal_ai_adj_{ds}", disabled=is_closed
            )
            ...
            if st.button("Salva pasto", key=f"meal_ai_save_{ds}", disabled=is_closed):
                if not adj_desc.strip():
                    ...
                    conn.execute(
                        "INSERT INTO meals (user_id, date, time, description, calories, raw_json) VALUES (?,?,?,?,?,?)",
                        (user_id, ds, m_time.strip(), adj_desc.strip(), float(adj_kcal), json.dumps(est, ensure_ascii=False)),
                    )
                    conn.commit()
    ...
    with tab2:
        ...
        if est:
            ...
            adj_kcal = st.number_input(
                "Kcal (modificabile)", min_value=0, value=int(round(float(est.get("total_calories") or 0))), step=10,
                key=f"photo_ai_adj_{ds}", disabled=is_closed
            )
            adj_desc = st.text_input(
            ...
            if st.button("Salva pasto (foto)", key=f"photo_ai_save_{ds}", disabled=is_closed):
                conn.execute(
                    "INSERT INTO meals (user_id, date, time, description, calories, raw_json) VALUES (?,?,?,?,?,?)",
                    (user_id, ds, p_time.strip(), adj_desc.strip(), float(adj_kcal), json.dumps(est, ensure_ascii=False)),
                )
                conn.commit()
    ...
    with tab3:
        ...
        if st.button("Salva pasto manuale", key=f"man_save_{ds}", disabled=is_closed):
            if not man_desc.strip():
                st.error("Inserisci una descrizione.")
            else:
                conn.execute(
                    "INSERT INTO meals (user_id, date, time, description, calories, raw_json) VALUES (?,?,?,?,?,?)",
                    (user_id, ds, man_time.strip(), man_desc.strip(), float(man_kcal), None),
                )
                conn.commit()
                st.rerun()

    # --- Allenamento (AI o manuale) ---
    with tab4:
            ...
            if st.button("Salva workout manuale", key=f"w_man_save_{ds}", disabled=is_closed):
                if not w_text.strip():
                    ...
                    conn.execute(
                        ...
                        (user_id, ds, w_time.strip(), w_text.strip(), int(w_dur or 0), kcal_burn, None),
                    )
                    conn.commit()
                    st.rerun()
        ...
        if est:
            ...
            if est.get("notes"):
                st.caption(f"📝 {est.get('notes')}")
            adj_kcal = st.number_input(
                "Kcal bruciate (modificabile)",
                min_value=0,
                value=int(round(float(est.get("calories_burned") or 0))),
                step=10,
                key=f"w_ai_adj_{ds}",
                disabled=is_closed,
            )
            if st.button("Salva workout (AI)", key=f"w_ai_save_{ds}", disabled=is_closed):
                conn.execute(
                    "INSERT INTO workouts (user_id, date, time, description, duration_min, calories_burned, raw_json) VALUES (?,?,?,?,?,?,?)",
                    (user_id, ds, w_time.strip(), w_text.strip(), int(w_dur or 0), float(adj_kcal), json.dumps(est, ensure_ascii=False)),
                )
                conn.commit()
